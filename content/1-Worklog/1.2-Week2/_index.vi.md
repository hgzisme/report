---
title: "Worklog Tuần 2"
date: 2025-09-09
weight: 1
chapter: false
pre: " <b> 1.2. </b> "
---

### Mục tiêu tuần 2:

* Học xong Module 2
* Hiểu được những gì có trong bài học và chuẩn bị làm lab để hiểu sâu hơn về cách thực hành, vận dụng trong thực tế

### Các công việc cần triển khai trong tuần này:
| Thứ | Công việc                                                                                                                                   | Ngày bắt đầu | Ngày hoàn thành | Nguồn tham khảo                                                                                                |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------- | ------------ | --------------- | -------------------------------------------------------------------------------------------------------------- |
| 2   | - Học Module 02 - 01                                                                                                                         | 15/9/2025    | 15/9/2025       | [Module 02 - 01](https://www.youtube.com/watch?v=O9Ac_vGHquM&list=PLahN4TLWtox2a3vElknwzU_urND8hLn1i&index=26) |
| 3   | - Học Module 02 - 02                                                                                                                         | 16/9/2025    | 16/9/2025       | [Module 02 - 02](https://www.youtube.com/watch?v=BPuD1l2hEQ4&list=PLahN4TLWtox2a3vElknwzU_urND8hLn1i&index=26) |
| 4   | - Học Module 02 - 03                                                                                                                         | 17/9/2025    | 17/9/2025       | [Module 02 - 03](https://www.youtube.com/watch?v=CXU8D3kyxIc&list=PLahN4TLWtox2a3vElknwzU_urND8hLn1i&index=27) |
| 5   | - Tìm hiểu thêm về các dịch vụ VPC <br>&emsp;+ VPC <br>&emsp;+ Subnet <br>&emsp;+ Route Table <br>&emsp;+ Elastic IP <br> &emsp;+ Nat Gateway <br/> - Tham gia sự kiện Vietnam Cloud Day 2025 : Ho Chi Minh City Connect Edition for Builders - Track 2: Migration and Modernization | 18/9/2025    | 18/9/2025       |                                                                                                                |
| 6   | - Học chương trình Coursera của trường đại học FPT                                                                                                              | 19/9/2025    | 19/9/2025       |                                                                                                                |

### Kết quả đạt được tuần 2:

* Đã tìm, học và hiểu về VPC 
  * Hiểu thêm về subnet: Public subnet, private subnet
  * Tìm hiểu về các dịch vụ của VPC:
    * Route table
    * Elastic Network Interface (ENI)
    * Elastic IP Address (EIP)
    * Internet Gateway
    * NAT Gateway
  * Tìm hiểu về các tính năng bảo mật của VPC:
    * Security Group
    * Network Access Control List (NACL)
    * Peering Connections (VPC Peering)
    * VPC Flowlogs
    * Transit Gateway
  * Tìm hiểu về các cách kết nối giữa môi trường on-premise/data center và AWS cloud
    * VPN Site-to-Site
    * Direct Connect
  * Tìm hiểu về dịch vụ cân bằng tải Elastic Load Balancer (ELB)
    * Tính năng Sticky Session (Session Affinity)
    * Application Load Balancer
    * Network Load Balancer
    * Classic Load Balancer
    * Gateway Load Balancer

* Hiểu về VPC Endpoint:
  * Hiểu được các khái niệm cơ bản về chúng
  * Hiểu về 2 loại Endpoint:
    * Interface Endpoint
    * Gateway Endpoint




